    ...
    <div className="app-shell flex min-h-screen">
      ...
      <main className="min-w-0 flex-1">
        ...
        <div className="page-content px-5 py-7 pb-24 md:px-9 md:py-9 md:pb-9">
          ...
          {page === "voice" && <VoicePage onNavigate={goTo} memories={[...personalMemories, ...familyMemories]} />}
...
function VoicePage({ onNavigate, memories }: { onNavigate: (page: Page) => void; memories: Array<SavedMemory | FamilyMemory> }) {
  const [listening, setListening] = useState(false);
  const [heard, setHeard] = useState("");
  const [supported, setSupported] = useState(true);
  const startListening = () => { const SpeechRecognition = (window as any).SpeechRecognition || (window as any).webkitSpeechRecognition; if (!SpeechRecognition) { setSupported(false); return; } const recognition = new SpeechRecognition(); recognition.lang = "en-US"; recognition.interimResults = true; recognition.onstart = () => setListening(true); recognition.onend = () => setListening(false); recognition.onresult = (event: any) => { const text = Array.from(event.results).map((result: any) => result[0].transcript).join(""); setHeard(text); const normalized = text.toLowerCase(); if (normalized.includes("memory bank") || normalized.includes("my memories")) onNavigate("memorybank"); else if (normalized.includes("family")) onNavigate("family"); else if (normalized.includes("memory game")) onNavigate("exercises"); else if (normalized.includes("tell me a memory") || normalized.includes("reminiscence")) onNavigate("reminiscence"); }; recognition.start(); };
  const readMemories = () => { if (memories.length && "speechSynthesis" in window) window.speechSynthesis.speak(new SpeechSynthesisUtterance(memories.slice(0, 4).map((memory) => `${memory.title}. ${memory.description}`).join(". "))); };
  return <div className="mx-auto max-w-[820px]"><div className="mb-7"><div className="eyebrow mb-2 text-[#d4873d]">Hands-free support</div><h1 className="display-font text-[38px] font-bold tracking-[-.04em] text-[#23463a]">Talk to MindMitra</h1><p className="mt-2 text-[14px] text-[#73847a]">Ask MindMitra to open a game, read your memories, or help you remember something.</p></div><section className="soft-card p-7 text-center sm:p-10"><div className={`mx-auto grid h-28 w-28 place-items-center rounded-full ${listening ? "bg-[#ffe5cf] text-[#d17731] shadow-[0_0_0_14px_rgba(212,135,61,.12)]" : "bg-[#e5f2e8] text-[#2c6e5b]"}`}><button onClick={startListening} className="grid h-20 w-20 place-items-center rounded-full bg-white shadow-sm" aria-label="Start voice recognition"><Mic size={34} /></button></div><div className="mt-6 text-[13px] font-bold text-[#4e6759]">{listening ? "Listening..." : "Tap the microphone to speak"}</div>{heard && <div className="mx-auto mt-4 max-w-[540px] rounded-[16px] bg-[#f4f8f4] p-4 text-left text-[13px] leading-relaxed text-[#5b7164]"><div className="eyebrow mb-2 text-[#94a49b]">You said</div>“{heard}”</div>}{!supported && <div className="mx-auto mt-4 max-w-[540px] rounded-[16px] bg-[#fff4e5] p-4 text-[12px] text-[#8e6f40]">Voice recognition is not supported in this browser. You can still use MindMitra normally.</div>}<div className="mt-8 grid gap-3 text-left sm:grid-cols-3"><VoiceAction icon={<NotebookPen size={18} />} title="Open my memory bank" onClick={() => onNavigate("memorybank")} /><VoiceAction icon={<Volume2 size={18} />} title="Read my memories" onClick={readMemories} /><VoiceAction icon={<Sparkles size={18} />} title="Tell me a memory" onClick={() => onNavigate("reminiscence")} /></div></section><div className="mt-5 rounded-[18px] border border-[#eadfce] bg-[#fff8ee] p-5 text-[12px] leading-relaxed text-[#8c7147]"><strong>Privacy note:</strong> Voice input stays in this browser prototype. MindMitra will always show what it heard before taking an important action.</div></div>;
}
function VoiceAction({ icon, title, onClick }: { icon: React.ReactNode; title: string; onClick: () => void }) { return <button onClick={onClick} className="flex items-center gap-3 rounded-[15px] border border-[#e4ece5] bg-white p-4 text-left transition hover:-translate-y-0.5 hover:shadow-sm"><div className="grid h-9 w-9 place-items-center rounded-xl bg-[#eef5ef] text-[#2c6e5b]">{icon}</div><span className="text-[12px] font-bold text-[#557061]">{title}</span></button>; }
