  ...
  return (
    <div className="app-shell flex min-h-screen">
      <aside className={`${mobileNavOpen ? "flex" : "hidden"} md:flex w-[238px] shrink-0 flex-col border-r border-[#dfe8e0] bg-[#f7faf6]/90 px-4 py-5 backdrop-blur-xl`}>
        ...
        <button onClick={() => goTo("settings")} className={`nav-item mt-1 flex w-full items-center gap-3 rounded-[13px] px-3 py-3 text-left text-[13px] font-semibold ${page === "settings" ? "bg-[#deede4] text-[#2c6e5b]" : "text-[#718278] hover:bg-[#edf4ef] hover:text-[#2c6e5b]"}`}>
          <SlidersHorizontal size={17} strokeWidth={1.8} /><span className="sidebar-label">Voice & accessibility</span>
        </button>
...
function DashboardPage({ profile, averageScore, averageAccuracy, streak, notes, results, recommendedDifficulty, recommendation, trainingPlan, dailyChallenge, personalMemories, familyMemories, onNavigate, onSelectGame }: { profile: Profile; averageScore: number; averageAccuracy: number; streak: number; notes: Note[]; results: Result[]; recommendedDifficulty: Difficulty; recommendation: any; trainingPlan: any[]; dailyChallenge: any; personalMemories: SavedMemory[]; familyMemories: FamilyMemory[]; onNavigate: (page: Page) => void; onSelectGame: (game: GameType) => void }) {
  ...
  return <div className="mx-auto max-w-[1180px]">
    <div className="mb-8 flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
      ...
      <div className="flex flex-wrap items-center gap-2"><ReadAloudControls text={`Good morning, ${name}. A little practice today keeps your attention clear and your memories close.`} /><button onClick={() => onSelectGame("memory")} className="flex w-fit items-center gap-2 rounded-[13px] bg-[#2c6e5b] px-4 py-3 text-[12px] font-bold text-white shadow-[0_8px_20px_rgba(44,110,91,.2)] transition hover:bg-[#245d4d]"><Play size={14} fill="currentColor" /> Start today’s practice <ArrowRight size={14} /></button></div>
    </div>
...
function MemoriesPage({ notes, setNotes, completedNotes }: { notes: Note[]; setNotes: React.Dispatch<React.SetStateAction<Note[]>>; completedNotes: number }) {
  ...
  return <div className="mx-auto max-w-[980px]"><div className="mb-7 flex flex-col justify-between gap-4 md:flex-row md:items-end"><div><div className="eyebrow mb-2 text-[#75a28b]">Memory assistance</div><h1 className="display-font text-[38px] font-bold tracking-[-.04em] text-[#23463a]">Keep it close</h1><p className="mt-2 text-[14px] text-[#73847a]">A private little shelf for reminders, ideas, and things worth returning to.</p></div><div className="flex flex-wrap gap-2"><ReadAloudControls text={`Memory assistance. ${notes.length} saved memories. ${notes.filter((note) => !note.completed).length} reminders are open.`} /><button onClick={() => setShowComposer((current) => !current)} className="flex w-fit items-center gap-2 rounded-[13px] bg-[#2c6e5b] px-4 py-3 text-[12px] font-bold text-white shadow-[0_8px_20px_rgba(44,110,91,.2)]"><Plus size={15} /> New memory</button></div></div>
...
function VoicePage({ onNavigate, memories, setMemories, onPersist }: { onNavigate: (page: Page) => void; memories: Array<SavedMemory | FamilyMemory>; setMemories: React.Dispatch<React.SetStateAction<SavedMemory[]>>; onPersist: (memory: SavedMemory) => void }) {
  ...
  const understandMemory = (text: string, detected: VoiceLanguage): SavedMemory => {
    ...
    const dateMatch = text.match(/(?:on|date|दिन|तारीख|తేదీ)\s+([^,.]+)/i);
    const timeMatch = text.match(/(?:at|by|around|बजे|సమయం)\s+([0-9]{1,2}(?::[0-9]{2})?\s?(?:am|pm|AM|PM)?)/i);
...
function CaretakerPage({ memories, familyMemories, results, notes, backendMemories, backendFamilyMemories, backendActivities }: { memories: SavedMemory[]; familyMemories: FamilyMemory[]; results: Result[]; notes: Note[]; backendMemories: SavedMemory[]; backendFamilyMemories: StoredFamilyMemory[]; backendActivities: any[] }) {
  ...
  const demoMemories: SavedMemory[] = [
    { id: "demo-1", title: "Morning garden memory", description: "The jasmine opened today and reminded me of home.", category: "Personal Memory", date: "2026-09-09", pinned: false, createdAt: "2026-09-09T08:10:00", language: "en-IN", source: "voice" },
  ...
  const alerts = daysSince > 4 ? ["No activity recently", "User has not added a memory for several days"] : ["User has been actively using MindMitra", "Voice memory recording is available in three languages"];
  return <div className="mx-auto max-w-[1180px]"><div className="mb-7 flex flex-col justify-between gap-4 lg:flex-row lg:items-end"><div><div className="eyebrow mb-2 text-[#8068a7]">Trusted family view</div><h1 className="display-font text-[38px] font-bold tracking-[-.04em] text-[#23463a]">Caretaker dashboard</h1><p className="mt-2 max-w-[680px] text-[14px] leading-relaxed text-[#73847a]">A simple engagement and activity view for an authorized family member. This is not medical monitoring.</p></div><div className="flex flex-wrap gap-2"><ReadAloudControls text="Caretaker dashboard. This view shows memory engagement, recent activity, language use, and simple alerts. It is not medical monitoring." /><button onClick={() => setDemoMode((value) => !value)} className={`rounded-xl px-4 py-3 text-[12px] font-bold ${demoMode ? "bg-[#eee9f8] text-[#8068a7]" : "bg-white text-[#718278] shadow-sm"}`}>{demoMode ? "Demo data on" : "Use real data"}</button></div></div>{!accessGranted ? <section className="dark-card grid-pattern p-7 sm:p-10"><div className="max-w-[620px]"><div className="mb-4 flex items-center gap-2 text-[11px] font-bold uppercase tracking-[.14em] text-[#d9c8e9]"><LockKeyhole size={15} /> Authorized access</div><h2 className="display-font text-[32px] text-white">Caretaker access is private.</h2><p className="mt-3 text-[14px] leading-relaxed text-[#c6d8ca]">This prototype uses a local access gate. In a connected version, replace this with secure authentication and explicit user consent.</p><button onClick={() => setAccessGranted(true)} className="mt-6 rounded-xl bg-white px-5 py-3 text-[13px] font-bold text-[#2c6e5b]">Enter caretaker view</button></div></section> : <><div className="mb-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><CaretakerMetric icon={<HeartPulse size={18} />} label="Memories added" value={activeMemories.length} tone="green" /><CaretakerMetric icon={<Mic size={18} />} label="Voice recorded" value={voiceMemories.length} tone="orange" /><CaretakerMetric icon={<Volume2 size={18} />} label="Read aloud uses" value={readCount} tone="purple" /><CaretakerMetric icon={<Activity size={18} />} label="Game sessions" value={results.length} tone="amber" /></div><div className="grid gap-5 xl:grid-cols-[1.2fr_.8fr]"><section className="soft-card p-6 sm:p-7"><div className="mb-5 flex items-center justify-between"><div><div className="eyebrow mb-1 text-[#90a49a]">Engagement over time</div><h2 className="display-font text-[24px] font-bold text-[#2a4a3d]">Weekly activity</h2></div><span className="rounded-full bg-[#e5f2e8] px-3 py-1.5 text-[10px] font-bold text-[#4f8e67]">{status.icon} {status.label}</span></div><div className="flex h-[180px] items-end gap-3 rounded-[16px] bg-[#f5f9f5] px-5 pb-5 pt-8">{[35, 54, 46, 76, 58, 88, Math.min(100, 44 + activeMemories.length * 8)].map((height, index) => <div key={index} className="flex flex-1 flex-col items-center gap-2"><div className="w-full rounded-t-lg bg-gradient-to-t from-[#70aa85] to-[#b4dcb9]" style={{ height: `${height}%` }} /><span className="text-[10px] font-bold text-[#94a299]">{["M", "T", "W", "T", "F", "S", "S"][index]}</span></div>)}</div><div className="mt-5 grid grid-cols-3 gap-3"><MiniStat label="This week" value={activeMemories.length + results.length} /><MiniStat label="Avg. sessions" value={Math.max(1, Math.round((activeMemories.length + results.length) / 7))} /><MiniStat label="Last active" value={lastActivity ? makeSafeDate(lastActivity) : "—"} /></div></section><section className="soft-card p-6 sm:p-7"><div className="mb-5"><div className="eyebrow mb-1 text-[#90a49a]">Activity alerts</div><h2 className="display-font text-[24px] font-bold text-[#2a4a3d]">What’s happening</h2></div><div className="space-y-3">{alerts.map((alert, index) => <div key={alert} className={`flex items-start gap-3 rounded-[14px] p-4 ${index === 0 && daysSince > 4 ? "bg-[#fff0ee] text-[#ae5d53]" : "bg-[#f1f7f2] text-[#5b7565]"}`}><span className="text-[18px]">{index === 0 && daysSince > 4 ? "🔴" : "🟢"}</span><div><div className="text-[12px] font-bold">{alert}</div><div className="mt-1 text-[11px] opacity-80">Engagement notification only — not a health prediction.</div></div></div>)}</div></section></div><div className="mt-5 grid gap-5 lg:grid-cols-2"><section className="soft-card p-6 sm:p-7"><div className="mb-5 flex items-center justify-between"><div><div className="eyebrow mb-1 text-[#90a49a]">Languages used</div><h2 className="display-font text-[23px] font-bold text-[#2a4a3d]">Voice activity</h2></div><Languages size={20} className="text-[#7fa48b]" /></div><div className="space-y-4">{Object.entries(languageCounts).map(([label, count]) => <div key={label}><div className="mb-1.5 flex justify-between text-[12px] font-bold text-[#61766a]"><span>{label}</span><span>{count}</span></div><div className="h-2 overflow-hidden rounded-full bg-[#edf3ee]"><div className="h-full rounded-full bg-[#73ad8e]" style={{ width: `${Math.min(100, count / Math.max(1, voiceMemories.length) * 100)}%` }} /></div></div>)}</div></section><section className="soft-card p-6 sm:p-7"><div className="mb-5 flex items-center justify-between"><div><div className="eyebrow mb-1 text-[#90a49a]">Recent activity</div><h2 className="display-font text-[23px] font-bold text-[#2a4a3d]">Memory timeline</h2></div><Clock3 size={20} className="text-[#7fa48b]" /></div><div className="space-y-3">{activeMemories.slice(0, 4).map((memory) => <div key={memory.id} className="flex items-center gap-3 rounded-xl bg-[#f7faf7] p-3"><div className="grid h-8 w-8 place-items-center rounded-lg bg-[#e5f2e8] text-[#4d9167]"><Mic size={15} /></div><div className="min-w-0 flex-1"><div className="truncate text-[12px] font-bold text-[#557061]">{memory.title}</div><div className="mt-1 text-[10px] text-[#97a59b]">{memory.language === "hi-IN" ? "Hindi" : memory.language === "te-IN" ? "Telugu" : memory.language === "en-IN" ? "English" : "Personal"} · {makeSafeDate(memory.createdAt)}</div></div></div>)}</div></section></div></>}</div>;
}

function CaretakerMetric({ icon, label, value, tone }: { icon: React.ReactNode; label: string; value: number; tone: "green" | "orange" | "purple" | "amber" }) { const styles = { green: "bg-[#e5f2e8] text-[#4f8d69]", orange: "bg-[#ffeadf] text-[#c76f4d]", purple: "bg-[#eee9f8] text-[#8069a2]", amber: "bg-[#fff0da] text-[#c78436]" }; return <div className="soft-card p-5"><div className={`mb-4 grid h-9 w-9 place-items-center rounded-xl ${styles[tone]}`}>{icon}</div><div className="text-[11px] font-semibold text-[#85948b]">{label}</div><div className="mt-1 display-font text-[30px] font-bold text-[#2a4a3d]">{value}</div></div>; }

function AccessibilitySettingsPage({ fontScale, setFontScale }: { fontScale: number; setFontScale: (value: number) => void }) {
  const [language, setLanguage] = useState(() => window.localStorage.getItem("mindmateVoiceSettings") || "en-IN");
  const [rate, setRate] = useState(() => Number(window.localStorage.getItem("mindmateSpeechRate") || "0.9"));
  const [readPreference, setReadPreference] = useState(() => window.localStorage.getItem("mindmateReadPreference") !== "off");
  const [voicePreference, setVoicePreference] = useState(() => window.localStorage.getItem("mindmateVoicePreference") !== "off");
  const save = (key: string, value: string) => window.localStorage.setItem(key, value);
  return <div className="mx-auto max-w-[900px]"><div className="mb-7"><div className="eyebrow mb-2 text-[#75a28b]">Accessibility and voice</div><h1 className="display-font text-[38px] font-bold tracking-[-.04em] text-[#23463a]">Make MindMitra comfortable</h1><p className="mt-2 text-[14px] leading-relaxed text-[#73847a]">Choose the language, text size, and speaking speed that feel easiest for you.</p></div><section className="soft-card p-6 sm:p-8"><div className="space-y-7"><SettingsRow title="Preferred language" description="Used for voice input and read aloud"><div className="grid gap-2 sm:grid-cols-3">{[["en-IN", "English"], ["hi-IN", "हिन्दी"], ["te-IN", "తెలుగు"]].map(([code, label]) => <button key={code} onClick={() => { setLanguage(code); save("mindmateVoiceSettings", code); }} className={`min-h-[58px] rounded-xl border-2 px-4 text-left text-[13px] font-bold ${language === code ? "border-[#2c6e5b] bg-[#e5f2e8] text-[#2c6e5b]" : "border-[#e4ece5] bg-white text-[#718278]"}`}>{label}</button>)}</div></SettingsRow><SettingsRow title="Speech speed" description="Slow it down for easier listening"><div className="flex items-center gap-3"><button onClick={() => { const next = Math.max(.6, Number((rate - .1).toFixed(1))); setRate(next); save("mindmateSpeechRate", String(next)); }} className="min-h-[52px] rounded-xl bg-[#eef5ef] px-4 text-[20px] font-bold text-[#2c6e5b]">−</button><div className="min-w-[90px] text-center text-[18px] font-bold text-[#456252]">{rate.toFixed(1)}×</div><button onClick={() => { const next = Math.min(1.4, Number((rate + .1).toFixed(1))); setRate(next); save("mindmateSpeechRate", String(next)); }} className="min-h-[52px] rounded-xl bg-[#eef5ef] px-4 text-[20px] font-bold text-[#2c6e5b]">+</button></div></SettingsRow><SettingsRow title="Text size" description="Larger text makes buttons and instructions easier to see"><div className="flex items-center gap-2"><button onClick={() => setFontScale(Math.max(.9, Number((fontScale - .05).toFixed(2))))} className="min-h-[52px] rounded-xl bg-[#eef5ef] px-4 text-[14px] font-bold text-[#2c6e5b]">A−</button><button onClick={() => setFontScale(1)} className="min-h-[52px] rounded-xl bg-[#eef5ef] px-4 text-[18px] font-bold text-[#2c6e5b]">A</button><button onClick={() => setFontScale(Math.min(1.15, Number((fontScale + .05).toFixed(2))))} className="min-h-[52px] rounded-xl bg-[#eef5ef] px-4 text-[22px] font-bold text-[#2c6e5b]">A+</button></div></SettingsRow><SettingsRow title="Read aloud preference" description="Keep spoken support available across important sections"><Toggle checked={readPreference} onChange={(value) => { setReadPreference(value); save("mindmateReadPreference", value ? "on" : "off"); }} /></SettingsRow><SettingsRow title="Voice input preference" description="Keep the large Speak button available"><Toggle checked={voicePreference} onChange={(value) => { setVoicePreference(value); save("mindmateVoicePreference", value ? "on" : "off"); }} /></SettingsRow></div></section><section className="soft-card mt-5 p-6 sm:p-7"><div className="flex items-start gap-3"><div className="grid h-10 w-10 place-items-center rounded-xl bg-[#e5f2e8] text-[#2c6e5b]"><LockKeyhole size={18} /></div><div><h2 className="display-font text-[22px] font-bold text-[#2a4a3d]">Privacy and access</h2><p className="mt-2 text-[13px] leading-relaxed text-[#73847a]">Personal memories and voice transcriptions stay in this browser. You can delete memories from the Memory Bank at any time. Caretaker view is an engagement prototype and should be connected to secure authentication before real sharing.</p></div></div></section></div>;
}
function SettingsRow({ title, description, children }: { title: string; description: string; children: React.ReactNode }) { return <div className="flex flex-col justify-between gap-4 border-b border-[#e8efe9] pb-6 last:border-b-0 last:pb-0 sm:flex-row sm:items-center"><div><div className="text-[15px] font-bold text-[#456252]">{title}</div><div className="mt-1 text-[12px] text-[#89988f]">{description}</div></div>{children}</div>; }
function Toggle({ checked, onChange }: { checked: boolean; onChange: (value: boolean) => void }) { return <button onClick={() => onChange(!checked)} className={`relative h-8 w-14 rounded-full transition ${checked ? "bg-[#2c6e5b]" : "bg-[#cfdad1]"}`} aria-label={checked ? "Enabled" : "Disabled"}><span className={`absolute top-1 h-6 w-6 rounded-full bg-white shadow-sm transition ${checked ? "left-7" : "left-1"}`} /></button>; }
